...
def spatial_to_channel(x, comp_ratio=(1, 1)):
    ...
    example:
    foo = torch.tensor([i for i in range(16)]).reshape((4,4,1))
    >> tensor([[[ 0],
         [ 1],
         [ 2],
         [ 3]],
    ...
    >> tensor([[[ 0,  1,  4,  5],
        ...
        [[ 8,  9, 12, 13], 
         [10, 11, 14, 15]]]) # shape (2,2,4)
    '''
    assert len(x.shape) == 3 or len(x.shape) == 4
    assert len(comp_ratio) == 2
    if len(x.shape) == 3:
        return rearrange(x, pattern='(h1 h2) (w1 w2) c -> h1 w1 (c h2 w2)', h2=comp_ratio[0], w2=comp_ratio[1])
    else:
        return rearrange(x, pattern='b c (h1 h2) (w1 w2) -> b (c h2 w2) h1 w1', h2=comp_ratio[0], w2=comp_ratio[1])


def channel_to_spatial(x, c=1):
    '''
    input:
    - x (Tensor, [B, k^2, H, W]) or (B, C, T ,H, W)
    return:
    - Tensor, [B, 1, H k, W k]
    '''
    assert len(x.shape) == 4 or len(x.shape) == 5
    ratio = int((x.shape[1]//c) ** 0.5)
    assert (ratio ** 2) == x.shape[1]//c, x.shape
    if len(x.shape) == 4:
        return rearrange(x, pattern='b (c h2 w2) h w -> b c (h h2) (w w2)', c=c, h2=ratio, w2=ratio)
    else:
        return rearrange(x, pattern='b (c h2 w2) t h w -> b c t (h h2) (w w2)', c=c, h2=ratio, w2=ratio)
